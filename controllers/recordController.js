...
const { getAmountByMonth, getAmountByCategory, organizeCategoryData, formatAmount } = require('../utils/records');
...
const recordController = {
	getRecords: async (req, res) => {
		try {
			...
			let monthOfYearSet = !selectedCategory && !selectedDate ? new Set() : req.session.monthOfYearSet.split(' ');
			let totalAmount = 0;
			let totalExpense = 0;
			let totalRevenue = 0;
			let { type } = req.params;
			...
			console.log('categoryList: ', amountByCategory);
			...
			// Format total amount
			[totalAmount, totalExpense, totalRevenue, formattedBudget, remaining] = formatAmount(
				totalAmount,
				totalExpense,
				totalRevenue,
				budget,
	...
	editRecord: async (req, res) => {
		try {
			const [record, categoryList] = await Promise.all([
				Record.findOne({ _id: req.params.id, userId: req.user._id }).populate('categoryId').lean().exec(),
